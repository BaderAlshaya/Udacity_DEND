...
The *user activity* data is in a directory of JSON logs and the *songs data* is in a directory of JSON songs metadata. Therefore, They'd like us to create a database with tables designed to optimize queries on song play analysis. We decided to create an SQL (relational) database schema and ETL pipeline for this analysis using Postgres.
